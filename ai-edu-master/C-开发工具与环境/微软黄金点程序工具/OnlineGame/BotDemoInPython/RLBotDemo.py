...
#定义QlearningTable类 强化学习表
class QLearningTable:
    def __init__(self, actions, learning_rate=0.01, reward_decay=0.9, e_greedy=0.9):
        self.actions = actions  # a list
        self.lr = learning_rate #学习率
        self.gamma = reward_decay  #衰变率
        self.epsilon = e_greedy  #贪心取值
        self.q_table = pd.DataFrame(columns=self.actions, dtype=np.float64) #定义pd表格，列数据为actions，类型为np的float64
...
# client = Client()
class server:
    def __init__(self, host, jsonpath):
        self.host = host
...
def main(roomId):
    se = server('https://goldennumber.aiedu.msra.cn/' ,'/swagger/v1/swagger.json')
    app = se.get_app()
    client = se.get_client()
    if roomId is None:
    ...
    while True:
        ...
        if len(todayGoldenList.goldenNumberList) != 0:
            print(todayGoldenList.goldenNumberList)
