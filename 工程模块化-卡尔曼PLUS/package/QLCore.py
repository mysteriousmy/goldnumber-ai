...
class QLTable:
    def __init__(self, actions, isUseLocalQLTable, learning_rate = 0.1, reward_decay = 0.9, greedy = 0.9, up = 0, down = 0):
    ...
    def getState(self,goldenNumberList):
        ...
        if len(goldenNumberList) == 0 or len(goldenNumberList) == 1:
            ...
            #这个方法有必要解释下，意思是迭代array每个符合条件的数字，并给他们都+1（+1的原因可能是负数存在的关系？），然后再sum下，就得出来了 其实这个上下界只是为了代表黄金点近期的浮动状态，并不精确
